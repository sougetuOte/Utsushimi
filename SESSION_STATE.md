...
最終更新：2026-09-23（Phase 1「設計」検収 PASS。Claude Code）
...
| 2 | 本ファイル | 索引 |
| 3 | `goal.md`（90行） | 直近フェーズの契約。今は Phase 1「設計」（検収 PASS 済み） |
...
| 9 | `docs/research/2026-09-23-phase1-spike.md`（87行） | ADR 0001 の根拠（3候補の実測）。必要なときだけ |
| 10 | `docs/research/2026-09-23-model-practices.md` | Opus 5.5 / Fable 5.1 の使い方。必要なときだけ |
...
- `spikes/`：使い捨てのスパイク（PySide6・Tauri・WPF）と計測の道具。**段1の T1 で本体が実起動したら消す**（研究文書の消滅条件）
...
- 本体を動かすには `.env`（`ANTHROPIC_API_KEY`）と uv 管理の Python 3.14（ADR 0001・0003）
...
| `c904a64`・`17a6a9f` | ADR 採用（主人承認）と段1の設計3点。columba 検収で V1〜V11 すべて PASS（1回目） |
...
**段1 T1「骨格」の面接（G0）。**材料は `docs/specs/stage1/tasks.md` の T1 と共通の完了条件、ADR 0001〜0004。
T1 は本物の API を呼ぶので、`.env` のキーが要る。
...
- **`spikes/tools/check.py` は約25秒マウスとキーボードを合成入力で動かす。**主人が PC を使っていないときに走らせる。
