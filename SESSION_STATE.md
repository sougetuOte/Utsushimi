...
最終更新：2026-09-24（段1 T2「人格の器」検収 PASS。Claude Code）
...
| 2 | 本ファイル | 索引 |
| 3 | `goal.md`（109行） | 直近フェーズの契約。今は段1 T2「人格の器」（検収 PASS 済み） |
...
| 8 | `docs/specs/stage1/`（requirements 96・design 214・tasks 87行） | 段1の設計。BH の振り分け・作り方・T1〜T8 |
| 9 | `docs/smoke-test.md`（93行） | 実起動の手順書と画面の目視チェックリスト。タスクごとに足す |
| 10 | `src/utsushimi/`（7本、約670行） | 本体。`ui`（画面）・`core`（核のスレッド）・`memory`（SQLite）・`llm`（API）・`persona`（人格のファイルの検査とハッシュ） |
...
- **本体（段1 T1）**：スタートメニューの `Utsushimi.lnk` から起動し、枠なしの窓で `claude-opus-5-5` と話せる。
  発言は1つずつ `data/utsushimi.db` に残り、強制終了のあとも欄に戻る。トレイで表示・隠す・終了
- **人格の器（段1 T2）**：`data/persona/core.md`・`style.md` を読んで検査し、壊れていれば小窓を出して止まる。
  `persona_seal` のハッシュと違う／まだ固めていなければ主人に問う（固め直しは行を足すだけ）。起動したらキャラが時間帯に合った挨拶をする。
  人格は確かめ用の仮のもの（Claude が手で書いた）。本物は T3 で生まれる
...
- キャラ作り・日記と検索・2重起動の防止・突っつき・最前面・長い返事の自動スクロール（T3〜T7）
...
| `fb7aaee`・`900be25` | 段1 T2「人格の器」の G0 と本体。T1 の試しのデータをごみ箱へ移してから確かめた（8回の実起動、証拠は `data/smoke/t2`、git の外）。時刻の挨拶の朝の分は翌朝に主人が判定（可）。検収役の検収で V1〜V13 すべて PASS（1回目） |
| `d11dff7` | 主人の指摘：長い返事は主人が自分でスクロールしないと末尾が読めない → T7 に完了条件を1行足した（T2 とは別コミット） |
...
- T2 の確かめで `data/` に溜まった仮の人格と会話（54発言）・ログ・スクリーンショットを消す（T3 の G0 で扱う。T3 は人格が無い状態から始まるため）

## 次の一手

**段1 T3「キャラ作り」の面接（G0）。**材料は `docs/specs/stage1/tasks.md` の T3 と共通の完了条件、`docs/concept.md` §8（キャラ体系）、design.md。
